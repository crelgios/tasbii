...
async function savePackFromInputs(){
  ...
  try{
    await savePacksLive();
    showToast("Live website updated. Open Daily Dhikr to check.");
  }catch(error){
...
async function deletePack(){
  ...
  try{
    await savePacksLive();
    showToast("Pack deleted from live Daily Dhikr");
  }catch(error){
...
async function loadCounterDuas(){
  ...
  return [
    {id:"durood_shareef", title:"Durood Shareef", arabic:"اللَّهُمَّ صَلِّ عَلَى مُحَمَّدٍ وَعَلَى آلِ مُحَمَّدٍ", meaning:"Send blessings upon Prophet Muhammad ﷺ", note:"Recite 100 times"}
  ];
}
...
function renderCounterDuas(){
  ...
  if(!counterDuas.length){
    counterDuas.push({id:"dua_" + Date.now(), title:"New Dua", arabic:"", meaning:"", note:"Recite 100 times"});
  }
  ...
  counterDuas.forEach((dua,index)=>{
    ...
    div.innerHTML = `
      ...
      <div class="pack-meta">100 recitations • ${escapeHtml(dua.meaning || dua.note || "No meaning added")}</div>
    `;
    div.onclick = () => {
...
function renderCounterEditor(){
  ...
  counterNote.value = dua.note || "Recite 100 times";
...
function renderCounterPreview(){
  ...
  counterPreview.innerHTML = `
    <div class="preview-card">
      ...
      <div class="small">100 recitations</div>
...
function syncCounterEditor(){
  ...
  dua.note = counterNote.value.trim() || "Recite 100 times";
...
async function saveCounterDuasLive(){
  try{
    ...
    renderCounterDuas();
    showToast("My Dua Counter updated live");
  }catch(err){
...
[counterTitle,counterArabic,counterMeaning,counterNote].forEach(el=>{ if(el) el.addEventListener("input",()=>{ syncCounterEditor(); renderCounterDuas(); renderCounterPreview(); }); });
